...
import { Users, ClipboardList, Eye, Pencil, Trash2, Send } from "lucide-react";
import { format } from "date-fns";

interface OperationsTeamOBPanelProps {
  onEntryCreated?: () => void;
}
...
const OperationsTeamOBPanel = ({ onEntryCreated }: OperationsTeamOBPanelProps) => {
  ...
  const fetchEntries = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('dob_entries')
        .select('*')
        .ilike('site_name', '%Operations Team%')
      ...
      const transformedEntries: OBEntry[] = (data || []).map((entry, index) => ({
        id: entry.id,
        entry_number: `OT-${String(index + 1).padStart(4, '0')}`,
  ...
  return (
    <div className="space-y-6">
      {/* Operations Team O.B Entry Form */}
                ...
                {loading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={9} className="text-center text-muted-foreground py-8">
                      No Operations Team entries found
                    </TableCell>
                  </TableRow>
                ) : (
                  entries.map((entry) => (
...
export default OperationsTeamOBPanel;
