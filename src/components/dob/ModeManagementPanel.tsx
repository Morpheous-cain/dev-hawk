...
import { Users, Shield, ClipboardList, Eye, Pencil, Trash2, Send, MapPin, Clock } from "lucide-react";
import { format } from "date-fns";

interface ModeManagementPanelProps {
  onEntryCreated?: () => void;
}
...
const operationsTeamRoles = ROLE_OPTIONS;
...
const ModeManagementPanel = ({ onEntryCreated }: ModeManagementPanelProps) => {
  const [activeMode, setActiveMode] = useState("operations_team");
  const [entries, setEntries] = useState<OBEntry[]>([]);
  const [loading, setLoading] = useState(false);

  // Operations Team Form State
  const [opsRole, setOpsRole] = useState("");
  const [opsOfficerName, setOpsOfficerName] = useState("");
  const [opsNature, setOpsNature] = useState("");
  const [opsSignature, setOpsSignature] = useState("");
  const [opsRemarks, setOpsRemarks] = useState("");
  const [opsEntryType, setOpsEntryType] = useState("");
  ...
  const fetchEntries = async () => {
    ...
    try {
      ...
      const transformedEntries: OBEntry[] = (data || []).map((entry, index) => ({
        id: entry.id,
        entry_number: `OB-${String(index + 1).padStart(4, '0')}`,
  ...
  const handleOpsSubmit = async () => {
    if (!opsRole || !opsOfficerName || !opsNature || !opsEntryType) {
    ...
    try {
      ...
      const roleLabel = operationsTeamRoles.find(r => r.value === opsRole)?.label;
      
      const { error } = await supabase.from('dob_entries').insert({
        entry_type: opsEntryType,
        site_name: `Operations Team - ${roleLabel}`,
        description: `[${roleLabel}] Officer: ${opsOfficerName} | ${opsNature}${opsRemarks ? ` | Remarks: ${opsRemarks}` : ''}${opsSignature ? ` | Signature: ${opsSignature}` : ''}`,
        recorded_by: userData.user?.id || opsOfficerName,
      });

      if (error) throw error;

      toast.success("Operations Team OB entry created successfully");
    ...
    <div className="space-y-6">
      <Tabs value={activeMode} onValueChange={setActiveMode} className="w-full">
        <TabsList className="grid w-full grid-cols-2 mb-6">
          <TabsTrigger value="operations_team" className="flex items-center gap-2">
            <Users className="h-4 w-4" />
            Operations Team O.B
          </TabsTrigger>
          <TabsTrigger value="field_ops" className="flex items-center gap-2">
        ...
        {/* Operations Team O.B */}
        <TabsContent value="operations_team" className="space-y-6">
          <Card className="border-border/50 bg-card/50 backdrop-blur">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <ClipboardList className="h-5 w-5 text-primary" />
                Operations Team O.B Entry
              </CardTitle>
              <p className="text-sm text-muted-foreground">
                For Supervisors, Rider Supervisors, Operations Officers, and Investigators
              </p>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={opsRole} onValueChange={setOpsRole}>
                    <SelectTrigger>
                      <SelectValue placeholder="Select role..." />
                    </SelectTrigger>
                    <SelectContent>
                      {operationsTeamRoles.map((role) => (
                ...
                <div className="space-y-2">
                  ...
                  <Select value={opsEntryType} onValueChange={setOpsEntryType}>
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Input 
                    value={opsOfficerName} 
                    onChange={(e) => setOpsOfficerName(e.target.value)}
                ...
                <div className="space-y-2">
                  ...
                  <Input 
                    value={opsSignature} 
                    onChange={(e) => setOpsSignature(e.target.value)}
              ...
              <div className="space-y-2">
                ...
                <Textarea 
                  value={opsNature} 
                  onChange={(e) => setOpsNature(e.target.value)}
              ...
              <Button onClick={handleOpsSubmit} className="w-full gap-2" disabled={opsSubmitting}>
                <Send className="h-4 w-4" />
                {opsSubmitting ? "Submitting..." : "Make OB Entry"}
              </Button>
            </CardContent>
          </Card>

          {/* Operations Team Entries Table */}
          <Card className="border-border/50 bg-card/50 backdrop-blur">
            <CardHeader>
              <CardTitle>Operations Team OB Entries</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        {/* Field Ops O.B */}
        <TabsContent value="field_ops" className="space-y-6">
          <Card className="border-border/50 bg-card/50 backdrop-blur">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
          ...
          <Card className="border-border/50 bg-card/50 backdrop-blur">
            <CardHeader>
              <CardTitle>Field Ops OB Entries</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {loading ? (
                      ...
                      entries.map((entry) => (
                        <TableRow key={entry.id}>
                          <TableCell>{entry.assignment_site || '-'}</TableCell>
                          <TableCell className="font-mono text-xs">{entry.entry_number}</TableCell>
                          <TableCell>{entry.date}</TableCell>
                          <TableCell>{entry.time}</TableCell>
                          <TableCell>{entry.officer_name}</TableCell>
